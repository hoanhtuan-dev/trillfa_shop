...
class DatabaseSeeder extends Seeder
{
    ...
    protected function presets(): void
    {
        ...
        // === Bộ dữ liệu chuyên dụng: Góc máy ảnh (camera), Ống kính (lens), Kịch bản quay (video) ===
        // [category, ui_label, prompt_injection, note (chú giải / ứng dụng tốt nhất)]
        $studio = [
            // Góc máy nhiếp ảnh (ẢNH tĩnh)
        ...
        // Xoá camera/lens/video_scene cũ rồi nạp lại theo bộ chuẩn hoá (mỗi gói có model riêng).
        Preset::whereIn('category', ['camera', 'lens', 'video_scene'])->delete();

        $sort = 0;
        $all = collect(array_merge($presets, $studio))->map(fn ($row) => [$row[0], $row[1], $row[2], $row[3] ?? '']);
