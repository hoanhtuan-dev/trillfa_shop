...
const query = ref(''), sortKey = ref('newest'), pquery = ref(''), gridCols = ref(4), pSort = ref('newest'), pCols = ref(4);
onMounted(loadRefs);
...
async function onFile(e) {
  ...
  const uploaded = [];
  for (const f of files) { const d = await store.uploadRef(f, false); if (d && d.url) uploaded.push({ url: d.url, name: f.name }); }
  await loadRefs();
  // Đưa TẤT CẢ ảnh vừa tải vào canvas cùng lúc (mỗi ảnh 1 layer nguồn).
  store.addImagesToCanvas(uploaded);
