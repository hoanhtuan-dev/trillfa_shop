...
function nav(d) {
  ...
  if (n) { store.viewer = n; resetZoom(); resetConfirm(); }
}
...
onBeforeUnmount(() => { window.removeEventListener('keydown', onKey, true); clearTimeout(confirmTimer); });
</script>
...
<template>
  <div class="fixed inset-0 z-[70] flex items-center justify-center bg-black/90 p-2 backdrop-blur-sm sm:p-5" @click.self="close">
    ...
    <!-- Chuyển ảnh -->
    <button v-if="items.length > 1" @click="nav(-1)" class="absolute left-2 top-1/2 z-30 hidden h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-ink-900/90 text-xl text-cream-100 transition hover:bg-brand-600 sm:grid" title="Ảnh trước (←)" aria-label="Ảnh trước">‹</button>
    <button v-if="items.length > 1" @click="nav(1)" class="absolute right-2 top-1/2 z-30 hidden h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-ink-900/90 text-xl text-cream-100 transition hover:bg-brand-600 sm:grid" title="Ảnh sau (→)" aria-label="Ảnh sau">›</button>
    ...
    <div class="flex h-full w-full max-w-7xl flex-col gap-2 lg:flex-row">
      ...
      <div class="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-2xl border border-ink-700/60 bg-ink-900/40">
        <div class="grid h-full w-full cursor-grab place-items-center overflow-hidden active:cursor-grabbing" style="touch-action:none"
             @wheel.prevent="onWheel"
             @pointerdown="panStart" @pointermove="panMove" @pointerup="panEnd" @pointerleave="panEnd">
          <img v-if="current?.media_url" :src="current.media_url" class="max-h-full max-w-full select-none object-contain transition-transform duration-75"
               :style="{ transform: 'translate(' + viewerPan.x + 'px, ' + viewerPan.y + 'px) scale(' + viewerZoom + ')' }" draggable="false" @dblclick="resetZoom" />
        ...
        <div v-if="current?.media_url" class="absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 items-center gap-0.5 rounded-full border border-ink-700 bg-ink-900/95 px-1.5 py-1 shadow-lg">
        ...
        <!-- Dải thumbnail (desktop) -->
        <div v-if="items.length > 1" class="absolute bottom-14 left-1/2 z-10 hidden max-w-[90%] -translate-x-1/2 items-center gap-1.5 overflow-x-auto rounded-2xl border border-ink-700 bg-ink-900/95 p-1.5 shadow-lg sm:flex">
          <button v-for="g in items" :key="g.id" @click="store.viewer = g; resetZoom()" class="relative h-11 w-11 shrink-0 overflow-hidden rounded-lg border-2" :class="current?.id === g.id ? 'border-brand-500' : 'border-ink-700/60 hover:border-ink-500'">
