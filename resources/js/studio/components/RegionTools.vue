  ...
  <div class="pointer-events-none absolute inset-x-0 bottom-16 z-40 flex justify-center lg:inset-auto lg:left-2 lg:top-1/2 lg:bottom-auto lg:-translate-y-1/2 lg:justify-start">
    <div class="scrollbar-hide pointer-events-auto flex max-w-[calc(100vw-1.5rem)] items-center gap-1 overflow-x-auto rounded-2xl border border-ink-700 bg-ink-900/85 p-1.5 shadow-xl backdrop-blur lg:max-w-none lg:flex-col lg:items-center lg:overflow-visible">
      ...
      <button @click="store.selectTool = !store.selectTool; store.finishDraw(); store.reframeOpen = false; store.filmOpen = false; store.exitErase(); store.clearInpaintMask()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Lựa chọn (quét chọn nhiều layer)" :aria-label="'Lựa chọn (quét chọn nhiều layer)'">
