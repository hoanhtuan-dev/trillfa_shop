...
<template>
  <div class="absolute left-2 top-1/2 z-40 -translate-y-1/2" v-if="store.upscaleSrc">
    <div class="flex flex-col items-center gap-1 rounded-2xl border border-ink-700 bg-ink-900/85 p-1.5 shadow-xl backdrop-blur">
      <button @click="reframeOpen = !reframeOpen; filmOpen = false; store.exitErase(); store.exitRegionSelect()"
      ...
      <button @click="store.toggleRegionSelect(); reframeOpen = false; filmOpen = false; store.exitErase()"
        :class="store.regionSelectMode ? 'bg-brand-600 text-white border-brand-400 shadow-brand-500/40' : 'text-cream-200 border-transparent hover:bg-ink-700'"
        class="grid h-11 w-11 place-items-center rounded-xl border transition-colors" title="Vẽ vùng chọn">
      ...
      <button @click="filmOpen = !filmOpen; reframeOpen = false; store.exitErase(); store.exitRegionSelect()"
      ...
      <button @click="store.toggleErase(); reframeOpen = false; filmOpen = false; store.exitRegionSelect()"
