  ...
  <div v-if="store.upscaleSrc" class="pointer-events-none absolute inset-x-0 bottom-16 z-40 flex justify-center lg:inset-auto lg:left-2 lg:top-1/2 lg:bottom-auto lg:-translate-y-1/2 lg:justify-start">
    <div class="scrollbar-hide pointer-events-auto flex max-w-[calc(100vw-1.5rem)] items-center gap-1 overflow-x-auto rounded-2xl border border-ink-700 bg-ink-900/85 p-1.5 shadow-xl backdrop-blur lg:max-w-none lg:flex-col lg:items-center lg:overflow-visible">
      <button @click="store.finishDraw(); store.reframeOpen = !store.reframeOpen; store.filmOpen = false; store.exitErase(); store.clearInpaintMask()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Reframe / Crop · Cắt khung theo tỷ lệ / chọn vùng">
        <svg class="h-[20px] w-[20px] lg:h-[22px] lg:w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2v14a2 2 0 0 0 2 2h14"/><path d="M18 22V8a2 2 0 0 0-2-2H2"/></svg>
      </button>
      ...
      <button @click="store.finishDraw(); store.startCanvasSelect('freehand'); store.reframeOpen = false; store.filmOpen = false; store.exitErase()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Vẽ vùng chọn tự do (lasso) · Xóa / Tô màu / Feather">
        <svg class="h-[20px] w-[20px] lg:h-[22px] lg:w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 19c0-7 2-11 6-13 1.5-.7 3-1 5-1"/><path d="M11 5c3-2 6-1 8 2 2 3 2 8-2 11-2.5 1.9-5.5 2-8 1"/><path d="M8 13c-1.5 1.5-2 3.5-1 5.5"/><path d="M3 21l4-4"/></svg>
      </button>
      <button @click="store.finishDraw(); store.startCanvasSelect('path'); store.reframeOpen = false; store.filmOpen = false; store.exitErase()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Vùng chọn bằng đường cong (path/curve) · click thêm điểm · Đóng để tạo vùng">
      ...
      <button @click="store.finishDraw(); store.startCanvasSelect('magic'); store.reframeOpen = false; store.filmOpen = false; store.exitErase()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Magic Wand · chọn vùng theo màu (click 1 điểm)">
        <svg class="h-[20px] w-[20px] lg:h-[22px] lg:w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21 2-2 2"/><path d="M15 6.6 3.3 18.3a2 2 0 0 0 0 2.8l2.6 2.6a2 2 0 0 0 2.8 0L20.4 12a2 2 0 0 0 0-2.8z"/><path d="m11 12 2 2"/></svg>
      </button>
      <button @click="store.toggleDraw(); store.reframeOpen = false; store.filmOpen = false; store.clearInpaintMask(); store.exitErase()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Vẽ tự do (brush)">
      ...
      <button @click="store.finishDraw(); store.toggleErase(); store.reframeOpen = false; store.filmOpen = false; store.clearInpaintMask()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Xóa vùng (feather)">
        <svg class="h-[20px] w-[20px] lg:h-[22px] lg:w-[22px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7 21-4.3-4.3a1 1 0 0 1 0-1.4l9.7-9.7a1 1 0 0 1 1.4 0l5.7 5.7a1 1 0 0 1 0 1.4L12 19"/><path d="M22 21H7"/><path d="m5 11 9 9"/></svg>
      </button>
      <button @click="store.finishDraw(); store.filmOpen = !store.filmOpen; store.reframeOpen = false; store.exitErase(); store.clearInpaintMask()"
        ...
        class="grid h-9 w-9 shrink-0 place-items-center rounded-xl border transition-colors lg:h-11 lg:w-11" title="Film Look · Gán tone màu phim">
