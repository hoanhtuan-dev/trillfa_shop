...
<template>
  <div class="absolute left-2 top-1/2 z-40 -translate-y-1/2" v-if="store.upscaleSrc">
    <div class="flex flex-col items-center gap-1 rounded-2xl border border-ink-700 bg-ink-900/85 p-1.5 shadow-xl backdrop-blur">
      <button @click="store.reframeOpen = !store.reframeOpen; store.filmOpen = false; store.exitErase(); store.clearInpaintMask()"
      ...
      <button @click="store.startCanvasSelect('rect'); store.reframeOpen = false; store.filmOpen = false; store.exitErase()"
      ...
      <button @click="store.startCanvasSelect('freehand'); store.reframeOpen = false; store.filmOpen = false; store.exitErase()"
      ...
      <button @click="store.toggleErase(); store.reframeOpen = false; store.filmOpen = false; store.clearInpaintMask()"
      ...
      <button @click="store.filmOpen = !store.filmOpen; store.reframeOpen = false; store.exitErase(); store.clearInpaintMask()"
