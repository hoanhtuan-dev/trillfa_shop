...
<template>
  <div class="card p-5" style="border:1px solid var(--color-brand-500); background: linear-gradient(160deg, rgba(80,150,150,.13), rgba(74,122,144,.06));">
    ...
    <p class="text-[11px] text-ink-500">Phân tích ảnh đang chọn (nhiều nguồn) → tạo prompt có cấu trúc.</p>
    ...
    <div v-if="store.suggestResult && (store.suggestResult.styles?.length || store.suggestResult.background || store.suggestResult.image_prompt_en)" class="mt-3 rounded-2xl border border-emerald-500/40 bg-emerald-900/25 p-3 text-xs">
