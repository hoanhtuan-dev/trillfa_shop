...
async function runUpscale() {
  ...
  try {
    ...
    const d = await store.api('/studio/upscale', { image: store.upscaleSrc, scale: Number(store.upscaleScale)||2, refine: Number(store.upscaleRefine)||0, photoreal: Number(store.studioPhotoreal)||0, light_shadow: Number(store.lightShadow)||0, sharpen: Number(store.sharpen)||0, clarity: Number(store.clarity)||0, vibrance: Number(store.vibrance)||0 });
    ...
    <div v-if="popupOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" @click.self="popupOpen=false">
      <div class="scrollbar-hide max-h-[92vh] w-full max-w-md overflow-y-auto rounded-3xl border border-brand-500/30 bg-ink-900 p-5" @click.stop>
        ...
        <div class="space-y-2.5">
          <div v-for="(f,i) in [['upscaleRefine','Tinh chỉnh AI'],['studioPhotoreal','Studio Chân thực'],['lightShadow','Ánh sáng & Bóng đổ'],['sharpen','Tăng nét chi tiết'],['clarity','Độ nổi khối (clarity)'],['vibrance','Màu sống động']]" :key="f[0]">
