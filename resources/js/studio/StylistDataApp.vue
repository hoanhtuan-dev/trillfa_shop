...
async function load() {
  ...
  } catch (e) { flash('Lỗi tải dữ liệu.', 'error'); }
...
<template>
  <div class="studio-dark mx-auto max-w-4xl p-5">
    <!-- toast -->
    <div v-if="toast" class="pointer-events-none fixed left-1/2 top-4 z-[90] -translate-x-1/2 rounded-full px-4 py-2 text-xs font-semibold shadow-2xl" :class="toastType === 'error' ? 'bg-red-600 text-white' : 'bg-ink-800 text-cream-100 border border-brand-500/40'">{{ toast }}</div>

    <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
      <div>
        <h1 class="font-display text-lg font-semibold text-cream-50">✨ Trợ lý thiết kế · Quản lý data</h1>
        <p class="mt-0.5 text-[11px] text-ink-500">Loại trang phục + ma trận câu hỏi dùng bởi card ✨ Trợ lý thiết kế trong Studio.</p>
      </div>
      <a href="/studio" class="btn-outline btn-sm whitespace-nowrap">← Về Studio</a>
    </div>
    ...
    <!-- Loại trang phục -->
    <div v-if="tab==='types'" class="card p-5">
      <div class="mb-3 flex items-center justify-between">
        <p class="text-xs text-ink-500">Ảnh đại diện được phục vụ tự động theo slug: <span class="text-brand-300">/garment/{slug}</span></p>
      ...
      <div v-else class="space-y-2">
        <div v-for="t in types" :key="t.id" class="flex items-center gap-3 rounded-xl border border-ink-700 bg-ink-800/60 p-2.5">
    ...
    <div v-else class="card p-5">
      ...
      <div v-if="loading" class="py-8 text-center text-xs text-cream-300/60">⏳ Đang tải…</div>
      <div v-else class="space-y-2">
        <div v-for="q in questions" :key="q.id" class="rounded-xl border border-ink-700 bg-ink-800/60 p-3">
