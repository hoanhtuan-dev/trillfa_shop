                ...
                >
                    <template v-for="child in item.children" :key="child.label">
                        <a
                            ...
                            :href="child.url"
                            class="block whitespace-nowrap rounded-2xl px-3 py-2 text-sm text-ink-700 transition-colors hover:bg-brand-600/10 hover:text-brand-700"
                        >{{ child.label }}</a>
                        <div v-else class="relative">
                            <a :href="child.url" class="flex items-center justify-between rounded-2xl px-3 py-2 text-sm text-ink-700 transition-colors hover:bg-brand-600/10 hover:text-brand-700">
        ...
        <div v-if="hasMore" ref="moreRef" class="relative">
            ...
            <div v-if="moreOpen" class="glass-strong absolute right-0 top-full z-50 mt-2 w-72 rounded-2xl p-2 shadow-xl">
                <template v-for="item in overflow" :key="item.label">
                    <a :href="item.url" class="block whitespace-nowrap rounded-2xl px-3 py-2 text-sm transition-colors hover:bg-brand-600/10 hover:text-brand-700" :class="isActive(item.url) ? 'bg-brand-600/10 text-brand-700' : 'text-ink-700'">{{ item.label }}</a>
                    <div v-if="hasChildren(item)" class="ml-3 border-l border-cream-200 pl-2">
                        <a
                            v-for="child in item.children"
                            :key="child.label"
                            :href="child.url"
                            class="block whitespace-nowrap rounded-2xl px-3 py-1.5 text-[13px] transition-colors hover:text-brand-700"
