...
@foreach($items as $item)
    ...
    <div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" class="relative">
        @if($hasChildren)
            <a href="{{ $item->url }}" class="flex items-center gap-1 {{ $level === 0 ? 'rounded-full px-4 py-2 text-sm font-medium text-ink-700 hover:bg-cream-200/70 hover:text-ink-900' : 'flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm text-ink-700 hover:bg-cream-100' }}">
