...
<head>
    ...
    @php
        ...
        $active = [
            ...
            'studio.presets' => 'Cài đặt (Prompt Templates)',
            'studio.api' => 'API',
        ];
    ...
    <div class="flex h-screen" x-data="{ sidebarCollapsed: false, navOpen: { ai: true, pm: true, catwalk: true, asset: true, sys: true } }">
        ...
        <aside class="hidden shrink-0 flex-col border-r border-ink-700 bg-ink-800 transition-[width] duration-300" :class="sidebarCollapsed ? 'lg:flex lg:w-16' : 'lg:flex lg:w-60'">
            <div class="flex items-center gap-2 border-b border-ink-700 px-3 py-3">
                ...
                <button @click="sidebarCollapsed = !sidebarCollapsed" class="ml-auto grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-ink-700 text-cream-300 transition-colors hover:bg-ink-700 hover:text-white" :title="sidebarCollapsed ? 'Mở rộng thanh bên' : 'Thu gọn thanh bên'"><span x-text="sidebarCollapsed ? '»' : '«'"></span></button>
            </div>
...
<div class="studio-nav-head flex cursor-pointer items-center justify-between" @click="navOpen.asset = !navOpen.asset" x-show="false">Asset Library <span x-text="navOpen.asset ? '▾' : '▴'"></span></div>
...
<div class="studio-nav-head flex cursor-pointer items-center justify-between" @click="navOpen.sys = !navOpen.sys" x-show="!sidebarCollapsed">Hệ thống <span x-text="navOpen.sys ? '▾' : '▴'"></span></div>
            ...
            <div class="border-t border-ink-700 px-3 py-3" x-show="!sidebarCollapsed">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('studio.presets') }}" class="link text-xs">Trợ giúp</a>
