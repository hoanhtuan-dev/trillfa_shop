        ...
        <aside class="lg:sticky lg:top-24 lg:self-start">
            <div class="card p-5">
                ...
                <nav class="mt-4 space-y-1">
                    ...
                    @endphp
                    @foreach($nav as $item)
                        <a href="{{ $item[1] }}" class="flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-medium transition {{ isset($active) && $active === $item[3] ? 'bg-brand-50 text-brand-800' : 'text-ink-700 hover:bg-cream-100' }}">
                ...
                <form method="POST" action="{{ route('logout') }}" class="mt-4 border-t border-cream-200 pt-4">
                    @csrf
                    <button type="submit" class="flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50">
